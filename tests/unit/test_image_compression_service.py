...
def test_compress_with_preset_returns_expected_shape():
    ...
    for preset, settings in PRESETS.items():
        compressed_data, content_type, quality = image_compression_service.compress_with_preset(
            file_data=data,
            preset=preset,
...
def test_compress_to_target_produces_quality_value():
    ...
    compressed_data, content_type, quality = image_compression_service.compress_to_target(
        file_data=data,
